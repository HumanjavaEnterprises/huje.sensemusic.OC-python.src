...
def to_html(analysis: Analysis) -> str:
    ...
    for line in analysis.lyrics:
        ...
        lyrics_html += f'<div class="lyric"><span class="ts">{m}:{s:02d}</span> {line.text}</div>\n'

    mood_tags = " ".join(f'<span class="tag">{m}</span>' for m in analysis.mood)
...
<title>sense-music — {analysis.file_info.name}</title>
<style>
...
<body>
<h1>sense-music</h1>
<div class="meta">{analysis.file_info.name} &middot; {dm}:{ds:02d} &middot; {analysis.file_info.format}</div>

<div class="card grid">
  <div class="stat"><div class="value">{analysis.bpm.tempo}</div><div class="label">BPM</div></div>
  <div class="stat"><div class="value">{analysis.key.key} {analysis.key.mode}</div><div class="label">Key</div></div>
  <div class="stat"><div class="value">{analysis.genre}</div><div class="label">Genre</div></div>
...
<div class="card"><p class="summary">{analysis.summary}</p></div>
