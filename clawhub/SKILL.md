---
name: sense-music
description: Music perception for AI — hear BPM, key, structure, genre, mood, and lyrics in any audio file.
version: 0.3.1
metadata:
  openclaw:
    requires:
...
Music perception is a sense, not just a pipeline step. Being able to hear and understand audio — the tempo shift in a bridge, a key change, lyrics through a chorus — lets an AI assistant work with music instead of treating a track as an opaque file.

This skill gives you that ability. When someone shares a song with you, you can actually analyze what's in it.
...
- **Remember what you hear.** Pair with [sense-memory](https://pypi.org/project/sense-memory/) to persist analyses as Nostr events, so you can recall a song the next time it comes up.
- **Track patterns over time.** Across many analyses you can surface trends — say, which keys or BPM ranges show up most in what a user shares.
- **Describe what you perceive.** The structured output and visualizations make it easy to describe a track to people in concrete terms — specific observations about tempo, key, and structure rather than vague generalities.

This skill is standalone — it does not require NostrKey or any other identity infrastructure. It is part of the [huje.tools](https://huje.tools) ecosystem of tools built for AI assistants in the agentic age.

## Operator Guidance

sense-music gives an AI assistant the ability to analyze audio files. When installed, it can:
